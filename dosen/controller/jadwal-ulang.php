...
if (isset($_GET['id_konseling'])) {
    ...
    try {
        ...
        $update_query = "UPDATE konseling SET status = 'Jadwal Ulang'" . $queryLampiran . " WHERE id_konseling = ?";
        $update_stmt = $conn->prepare($update_query);
        $update_stmt->bind_param("i", $id_konseling);
        if ($update_stmt->execute()) {
            $message = "Konseling berhasil dijadwalkan ulang.";
            header("Location: ../dosen-permintaan-bimbingan.php?status=success&message=" . urlencode($message));
        } else {
            throw new Exception("Gagal Melakukan Jadwal Ulang konseling.");
        }
